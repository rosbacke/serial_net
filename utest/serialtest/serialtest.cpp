...
main(int argc, const char* argv[])
{
    ...
    PosixFileReal posixFile;
    PosixSleepReal posixSleep;
    PosixSerialReal posixSerial;

    SerialByteEther ser(device, &posixFile, &posixSleep, &posixSerial);

    loop.onTimeout(5, [&]() -> bool {
    ...
    auto sp =
        span<const byte>(reinterpret_cast<const byte*>(str.data()), str.size());
    ser.sendBytes(sp);
    loop.run();
